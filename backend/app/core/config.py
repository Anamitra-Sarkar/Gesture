...
class Settings(BaseSettings):
    ...
    def allowed_origins(self) -> List[str]:
        ...
        if not env_origins:
            # Normalize default origins to include both versions
            normalized_defaults = {}
            for origin in self._default_cors_origins:
                origin_no_slash = origin.rstrip('/')
                if origin_no_slash:
                    normalized_defaults[origin_no_slash] = None
                    normalized_defaults[origin_no_slash + '/'] = None
            return list(normalized_defaults.keys())
